...
const PreviewApartment = "/images/maps/standalone/test.jpg";
...
const CreatePrivateForm = () => {
	...
	const submitForm = () => {
		...
		axios
			.post(window.location.origin + "/api/createRoom", {
				name: roomName,
				password: password,
	...
	let createRoomDescription = (
		<div
			...
			<p>
				To create a private room on Online Town, you just need to fill out a few
				settings. When you’re finished, only the people you share the link and
				password with can hang out with you in your space. Rooms on Online Town
				don’t expire and there’s no limit to how many people you can invite or
				how long you can spend there.
			</p>
		</div>
	);
	...
	let addRoomName = (
		<div
			...
			<div
				...
				{name !== "" ? <i className="fas fa-check green checkmark"></i> : <></>}
				<h3>1) Room Name</h3>
			</div>
			<p>
				All room names welcome here. Just keep in mind that anyone you share the
				room with will be able to see the room name and it will show up in the
				URL as well
			</p>
	...
	let addEnvironment = (
		<div style={{ marginTop: "20px", maxWidth: "424px", lineHeight: "24px" }}>
			<div
				...
				{map !== 0 ? <i className="fas fa-check green checkmark"></i> : <></>}
				<h3>2) Environment</h3>
			</div>
	...
	let addPassword = (
		<div
			...
			<div
				...
				<h3>
					3) Room Password <span style={{ color: "gray" }}>(Optional)</span>
				</h3>
			</div>
			<p>
				Your room URL is protected by a long and random string of characters. We
				recommend that you consider also adding a password if you want
				additional security:{" "}
			</p>
	...
	let addModeratorPassword = (
		<div
			...
			<div
				...
				<h3>
					4) Moderator Password{" "}
					<span style={{ color: "gray" }}>(Optional)</span>
				</h3>
			</div>
			<p>
				If you’d also like to enable moderator settings for your room, you’ll
				need to add a moderator password. Once your room is created, anyone with
				access to the moderator password can function as a host, which is useful
				for organizing events or parties.
			</p>
			<div className="space-container">
				<input
					type="password"
	...
	let createButton = (
		<div style={{ marginBottom: "30px" }}>
			...
			{error ? (
				<div className="red" style={{ marginTop: "10px" }}>
					{error}
				</div>
			) : null}
		</div>
...
export default function CreatePrivate() {
	return (
		<div className="vertical-center-container">
			<GameHeader showCreateNewRoom={false} />
			<CreatePrivateForm />
		</div>
	);
}
