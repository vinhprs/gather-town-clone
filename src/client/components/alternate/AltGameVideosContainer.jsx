...
export default function AltGameVideosContainer(props) {
	...
	useEffect(() => {
		...
		let closestIds = Object.keys(props.playerVideoMap["playerToDist"])
			.filter((playerId) => {
				return (
					...
					playerId !== props.playerVideoMap["announcerPlayer"] + ""
				);
			})
			.sort((a, b) => {
				return props.playerVideoMap["playerToDist"][a] <
					props.playerVideoMap["playerToDist"][b]
					? -1
					: 1;
			})
			.slice(0, maxVideos);
		...
		Object.keys(streamMap).forEach((id) => {
			let persistentId =
				id in props.playerInfoMap
					? props.playerInfoMap[id]["publicId"]
					: "NOTANID";
			let idBlocked =
				persistentId in props.blocked ? props.blocked[persistentId] : false;
			let idVideoEnabled =
				id in otherVideoEnabled ? otherVideoEnabled[id] : true;
			let idAudioEnabled =
				id in otherAudioEnabled ? otherAudioEnabled[id] : true;
		...
		Object.keys(ownStreamMap).forEach((id) => {
			...
			if (id === props.myPlayerId) {
				...
				ownStreamMap[id].getVideoTracks().forEach((track) => {
					track.enabled =
						ownVideoEnabled && !idBlocked && inStreamingDistance(id);
				});
				ownStreamMap[id].getAudioTracks().forEach((track) => {
					track.enabled =
						ownAudioEnabled && !idBlocked && inStreamingDistance(id);
				});
			}
		});
	}, [
		maxVideos,
		props.playerVideoMap,
		props.playerInfoMap,
		props.myPlayerId,
		props.blocked,
		props.videoThreshold,
		streamMap,
		ownStreamMap,
		ownVideoEnabled,
		ownAudioEnabled,
		...
		navigator.mediaDevices
			...
			.then((devices) => {
				...
				if (notIR.length < devices.length) {
					let frontDevices = notIR.filter((device) =>
				...
				if (connectDeviceId) {
					mediaSettings.video = Object.assign(mediaSettings.video, {
	...
	function getGameVideo(playerId) {
		let persistentId =
			playerId in props.playerInfoMap
				? props.playerInfoMap[playerId]["publicId"]
				: "NOTANIDSET";
		...
		return (
			<>
				<GameVideo
					...
					distance={distance}
					videoEnabled={
						playerId in otherVideoEnabled ? otherVideoEnabled[playerId] : true
					}
					audioEnabled={
						playerId in otherAudioEnabled ? otherAudioEnabled[playerId] : true
					}
					blocked={
						persistentId in props.blocked ? props.blocked[persistentId] : false
					}
					setVideoEnabled={(enabled) =>
						setOtherVideoEnabled({ ...otherVideoEnabled, [playerId]: enabled })
					}
	...
	let errorComponent = (
		<div style={{ color: "red", marginTop: "10px" }}>
			Could not get your camera. Online Town requires audio and video to work
			properly! If you don't want to be on audio/video right now, you can check
			out our
			<a style={{ color: "orangered" }} target="_blank" href="/about.html">
				about
			</a>
	...
	let videoComponents = (
		<>
			{props.playerVideoMap["announcerPlayer"] &&
			props.playerVideoMap["announcerPlayer"] !== props.myPlayerId
	...
	let message1 = (
		<div>
			<p>
				You can't see/hear some people around you because there's too many
				people around.
			</p>
			<p>
				If you think your computer can handle it, increase the max connections
				to see them.
			</p>
		</div>
	);
	...
	let videoOptions = (
		<div className="videos-max-connections mobileHide">
			...
			{maxVideos < otherVideoComponents.length ? message1 : message2}
		</div>
	);
